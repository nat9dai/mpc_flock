# ROS Package distributed_nmpc_controller


## Installation and Setup

Move the auto-generated ROS package (folder `distributed_nmpc_controller`) to your catkin workspace (typically `~/catkin_ws/`):
...
```
roslaunch distributed_nmpc_controller open_optimizer.launch
```

Once you run the node, you can post your request to `distributed_nmpc_controller/parameters` as follows:

```
rostopic pub /distributed_nmpc_controller/parameters  \
  distributed_nmpc_controller/OptimizationParameters  \
  "{'parameter':[YOUR_PARAMETER_VECTOR],       \
   'initial_guess':[INITIAL_GUESS (OPTIONAL)]}" --once
```

The result will be announced on `/distributed_nmpc_controller/result`:

```
rostopic echo /distributed_nmpc_controller/result
```

To get the optimal solution you can do

```
rostopic echo /distributed_nmpc_controller/result/solution
```
